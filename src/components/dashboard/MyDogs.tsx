...
export default function MyDogs({ dogs, profile, onSave }: { dogs: any[], profile: any, onSave: () => void }) {
  ...
  // DD/MM/YYYY masked input handler
  ...
  async function handleStatusChange(dogId: string, newStatus: string) {
    ...
    const { error } = await supabase.from('dogs').update({ status: newStatus }).eq('id', dogId)
  ...
  async function handleDelete(dogId: string) {
    setDeletingId(dogId)
    // Check if dog has competition results
    ...
    if (results && results.length > 0) {
      ...
      alert(t(
        'Αυτός ο σκύλος έχει αποτελέσματα αγώνων και δεν μπορεί να διαγραφεί. Αλλάξτε την κατάστασή του σε "Αποσυρμένος" ή "Στη μνήμη του".',
        'This dog has competition results and cannot be deleted. Change their status to "Retired" or "In our memories" instead.'
      ))
      setDeletingId(null)
      return
    }
