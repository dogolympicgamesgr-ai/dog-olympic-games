      ...
      <div style={{ maxWidth: '900px', margin: '0 auto', padding: '0 1.5rem' }}>
        <div style={{ textAlign: 'center', marginBottom: '2.5rem' }}>
          ...
          <p style={{ color: 'var(--text-secondary)', fontSize: '0.88rem', marginTop: '0.5rem' }}>
            {t('Γνωρίστε τους decoys των αγώνων.', 'Meet the decoys of the games.')}
          </p>
        </div>
        ...
        {loading ? (
          ...
          <div style={{ textAlign: 'center', color: 'var(--text-secondary)', fontSize: '0.9rem' }}>
            {t('Δεν υπάρχουν decoys ακόμα.', 'No decoys yet.')}
          </div>
        ) : (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '1.25rem' }}>
            {members.map(m => (
              <div
                key={m.member_id}
                onClick={() => router.push(`/decoys/${m.member_id}`)}
                style={{
