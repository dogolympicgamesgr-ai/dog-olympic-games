...
export default function TeamProfilePage({ params }: { params: { id: string } }) {
  ...
  async function load() {
    ...
    const { data: teamData } = await supabase
      .from('teams').select('*').eq('id', params.id).single()
    ...
    // Get members with dogs
    const { data: memberRows } = await supabase
      .from('team_members')
      .select('user_id, profiles(id, full_name, avatar_url, member_id, points)')
      .eq('team_id', params.id)
      .eq('status', 'accepted')

    // For each member, get their dogs
    const enriched = await Promise.all((memberRows || []).map(async (m: any) => {
      const { data: dogs } = await supabase
        .from('dogs')
        .select('id, name, avatar_url, breed_id, breeds(name_el, name_en)')
        .eq('owner_id', m.user_id)
      return { ...m, dogs: dogs || [] }
    }))

    setMembers(enriched)
    const pts = enriched.reduce((sum, m) => sum + (m.profiles?.points || 0), 0)
    setTotalPoints(pts)
    ...
    if (user) {
      ...
      // Does user have pending request to THIS team?
      const { data: pending } = await supabase
        .from('team_members').select('id')
        .eq('user_id', user.id).eq('team_id', params.id).eq('status', 'pending').maybeSingle()
  ...
  async function handleJoinRequest() {
    ...
    const { error } = await supabase.from('team_members').insert({
      team_id: params.id, user_id: currentUserId,
    ...
    if (error) {
      ...
      if (captain?.id) {
        const { data: senderProfile } = await supabase
          .from('profiles').select('full_name, member_id')
        ...
        await supabase.from('notifications').insert({
          ...
          metadata: { team_id: params.id, requester_id: currentUserId },
        })
      }
    }
    setJoining(false)
    ...
    <main style={{
      ...
      <div style={{ maxWidth: '700px', margin: '0 auto', padding: '0 1.5rem' }}>
        ...
        <div style={{
          ...
          {team.avatar_url ? (
            <img src={team.avatar_url} style={{ width: 90, height: 90, borderRadius: '50%', objectFit: 'cover', border: '3px solid var(--accent)', marginBottom: '1rem' }} />
          ) : (
          ...
          {captain && (
            <div style={{
              ...
              background: 'var(--bg)', borderRadius: '99px', padding: '0.35rem 0.85rem',
              cursor: 'pointer',
          ...
          {/* Join button */}
          {currentUserId && !userInTeam && !userHasPending && currentUserId !== team.created_by && (
          ...
          {currentUserId && userInTeam && !members.some(m => m.user_id === currentUserId) && (
            ...
            <div key={m.user_id} style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: m.dogs.length ? '0.75rem' : 0, cursor: 'pointer' }}
                ...
                {m.profiles?.avatar_url ? (
                  <img src={m.profiles.avatar_url} style={{ width: 40, height: 40, borderRadius: '50%', objectFit: 'cover' }} />
                ) : (
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem', paddingTop: '0.5rem', borderTop: '1px solid var(--border)' }}>
                  {m.dogs.map((dog: any) => (
                    <div key={dog.id} onClick={() => router.push(`/dogs/${dog.id}`)}
                      ...
                      {dog.avatar_url ? (
                        <img src={dog.avatar_url} style={{ width: 24, height: 24, borderRadius: '50%', objectFit: 'cover' }} />
                      ) : (
