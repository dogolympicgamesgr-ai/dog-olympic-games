...
export default function JudgePanelPage() {
  ...
  useEffect(() => {
    async function init() {
      ...
      if (!data.roles?.includes('judge')) { router.push('/dashboard'); return }
      setSession(data)
      setChecking(false)
    ...
    const { data } = await supabase
      ...
      .select(`
        ...
        events!event_assignments_event_id_fkey(
          id, title_el, title_en, location, event_date, status
        ),
  ...
  async function respond(assignmentId: string, newStatus: 'accepted' | 'declined', eventCreatedBy: string | null, eventTitle: string) {
    ...
    // Notify organizer
    if (eventCreatedBy) {
      const responderName = session?.profile?.full_name || 'A judge'
      ...
      await supabase.from('notifications').insert({
        ...
        title_el: `Κριτής ${statusEl} την πρόσκληση`,
        title_en: `Judge ${statusEn} the invitation`,
        message_el: `${responderName} ${statusEl} την πρόσκληση ως κριτής για τον αγώνα "${eventTitle}".`,
        message_en: `${responderName} ${statusEn} the invitation as judge for event "${eventTitle}".`,
        metadata: { role: 'judge' },
      })
    }
                      ...
                      <div key={a.id} style={{ ...cardStyle, border: '1px solid var(--accent)44' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '1rem', flexWrap: 'wrap' }}>
                          <div style={{ flex: 1 }}>
                            ...
                            {cat && (
                              <p style={{ margin: '0 0 0.2rem', fontSize: '0.78rem', color: 'var(--accent)' }}>
                                ⚖️ {t('Κατηγορία', 'Category')}: {t(cat.title_el, cat.title_en || cat.title_el)}
                              </p>
                            )}
                            {ev?.event_date && (
            ...
            <div>
              ...
              {past.length === 0
                ...
                : past.map(a => {
                    const ev = a.events
                    const cat = a.event_categories
                    const canScore = ev?.status === 'completed'
                    return (
                      <div key={a.id} style={cardStyle}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '1rem', flexWrap: 'wrap' }}>
                          <div style={{ flex: 1 }}>
                          ...
                          <div style={{ display: 'flex', gap: '0.5rem', flexShrink: 0 }}>
                            {canScore && (
                              <button
                                onClick={() => router.push(`/events/${ev?.id}/score`)}
                                style={{ background: 'rgba(212,175,55,0.15)', border: '1px solid var(--accent)', borderRadius: '8px', padding: '0.4rem 0.9rem', color: 'var(--accent)', cursor: 'pointer', fontSize: '0.82rem', fontFamily: 'Outfit, sans-serif', fontWeight: 700, whiteSpace: 'nowrap' }}
                              >
                                ⚖️ {t('Βαθμολόγηση →', 'Score →')}
                              </button>
                            )}
                            <button
                              onClick={() => router.push(`/events/${ev?.id}`)}
                              style={{ background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: '8px', padding: '0.4rem 0.9rem', color: 'var(--text-secondary)', cursor: 'pointer', fontSize: '0.82rem', fontFamily: 'Outfit, sans-serif', whiteSpace: 'nowrap' }}
                            >
                              {t('Προβολή →', 'View →')}
                            </button>
                          </div>
